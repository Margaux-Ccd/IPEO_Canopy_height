...
def visualize_images(val_dataset, num_images):
    ...
    for i in range(num_images):
        ...
        print(f"Index of image to visualize = {idx}")

        data, target = val_dataset[idx] 

        # Normalize and prepare RGB image for visualization
        ...
        data = data.unsqueeze(0)  # Shape: (1, channels, height, width)
        ...
        # convert to numpy 
        output_2d = output_image.squeeze().detach().numpy()  # Shape : (H, W)

        # Plot Input Image (RGB)
        axes[i][0].imshow(rgb_normalized.permute(1, 2, 0).numpy())  # CHW -> HWC
